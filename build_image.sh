...
distributor_path = Path(sys.argv[2])
...
if contract.get("platform") != sys.argv[5]:
    raise SystemExit("Contract artifact platform is invalid")
verify_files(contract_path.parent, contract)
if distributor.get("package") != "rl-model-distributor" or distributor.get("version") != sys.argv[4]:
    raise SystemExit("ModelDistributor artifact identity is invalid")
if distributor.get("platform") != sys.argv[5]:
...
actual = (
    distributor["contract"]["version"],
    distributor["contract"]["source_id"],
    distributor["contract"]["source_sha256"],
)
if actual != expected:
    raise SystemExit(
        "ModelDistributor artifact was built against a different contract artifact"
    )
PY
