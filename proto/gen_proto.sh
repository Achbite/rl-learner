...
if ! test -f "${artifact_dir}/schemas/maze.metrics.v3.json" ||
   ! test -f "${artifact_dir}/schemas/maze.metrics.v3.sha256"; then
...
cp "${artifact_dir}"/schemas/maze.metrics.v3.json \
   "${artifact_dir}"/schemas/maze.metrics.v3.sha256 \
   "${output_dir}/schemas/"
printf '%s\n' "${output_dir}"
